...
N_POINTS = 4
POINT_2D_SIZE = 8
POINT_2D_ROW_OFFSET = 0
POINT_2D_COL_OFFSET = 4
...
points_loop_cond:
	...
	la	$t4, my_points
	mul	$t5, $t0, POINT_2D_SIZE		# i * sizeof(element)
	add	$t4, $t4, $t5			# &my_points[i]		

					# int row = my_points[i].row;
	lw	$t1, POINT_2D_ROW_OFFSET($t4)
	
					# int col = my_points[i].col;
	lw	$t2, POINT_2D_COL_OFFSET($t4)

					# int height = topography_grid[row][col];
	mul	$t4, $t1, 20		# $t4 = row*(size of a row)
	mul	$t5, $t2, 4		# col * sizeof(element)
	add	$t4, $t5, $t4		
